...
string rom_to_arab(string rom){
	unsigned idx = 0;
	int answer = 0;
	if(idx + 4 <= rom.length() && rom[idx] == 'M' && rom[idx + 1] == 'M' && rom[idx + 2] == 'M' && rom[idx + 3] == 'M'){
		answer += 4000;
		idx += 4;
	}else if(idx + 3 <= rom.length() && rom[idx] == 'M' && rom[idx + 1] == 'M' && rom[idx + 2] == 'M'){
		answer += 3000;
		idx += 3;
...
int main(){
	...
	while(cin >> s){
		if(s == "=" && calc.size() >= 1){
			int x = calc.top();
			if(x > 0 && x <= 4999){
				cout << arab_to_rom(to_string(x));
			}else{
				cout << "out of range exception";
			}
			cout << "\n";
		}else if(s == "+" && calc.size() >= 2){
			int first = calc.top();
			calc.pop();
			int second = calc.top();
			calc.pop();
		...
		}else if(((s == "+" || s == "-" || s == "*" || s == "/") && calc.size() < 2)
		|| (s == "=" && calc.size() < 1)){
			cout << "stack underflow\n";
		}else{
			calc.push(stoi(rom_to_arab(s)));
		}
	}
	return 0;
}
